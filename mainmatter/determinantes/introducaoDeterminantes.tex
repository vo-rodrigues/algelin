...
\begin{lemma}
    ...
    Seja $j\in \{1, \ldots, n+1\}$.
    Então a função $f: (R^{n+1})^{n+1} \to R$ definida por
    \begin{equation*}
        f(v_1, v_2, \ldots, v_{n+1}) = \sum_{j=1}^{n+1} (-1)^{j+i} v_{ij} g(v_1^{(j)}, \dots, v_{i-1}^{(j)}, v_{i+1}^{(j)}, \ldots, v_{n+1}^{(j)}),
    \end{equation*}

    em que $v_{ij}$ é a $j$-ésima coordenada do vetor $v_i$ e $v_k^{(j)}$ é o vetor obtido de $v_k$ ao remover sua $j$-ésima coordenada, é multilinear e alternada.
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{aligned}
            f(v_1, \ldots, v_{k-1}, v_k + \alpha v_k', v_{k+1}, \ldots, v_{n+1}) &= \sum_{j=1}^{n+1} (-1)^{j+i} v_{ij} g(v_1^{(j)}, \ldots, (v_k + \alpha v_k')^{(j)}, \ldots, v_{n+1}^{(j)})\\
            &= \sum_{j=1}^{n+1} (-1)^{j+i} v_{ij} \big(g(v_1^{(j)}, \ldots, v_k^{(j)}, \ldots, v_{n+1}^{(j)}) + g(v_1^{(j)}, \ldots, \alpha v_k'^{(j)}, \ldots, v_{n+1}^{(j)})\big)\\
