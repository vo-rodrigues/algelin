...
\begin{lemma}
    Para todo $n\geq 1$, existe no máximo uma função multilinear e alternada $f: (R^n)^n \to R$, e esta é dada por:

    \begin{equation*}
        f(v_1, v_2, \ldots, v_n) = \sum_{\sigma \in S_n} \sgn(\sigma) \prod_{i=1}^n v_{i\sigma(i)}.
    \end{equation*}
    Acima, $\sgn(\sigma)$ é a paridade do número de transposições necessárias para obter a permutação $\sigma$ a partir da permutação identidade, $v_{ij}$ é a $j$-ésima coordenada do vetor $v_i$, e $S_n$ é o conjunto de todas as permutações de $\{1, 2, \ldots, n\}$.
\end{lemma}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{aligned}
            f(v_1, v_2, \ldots, v_n) &= f\left(\sum_{j_1=1}^n v_{1j_1} e_{j_1}, \sum_{j_2=1}^n v_{2j_2} e_{j_2}, \ldots, \sum_{j_n=1}^n v_{nj_n} e_{j_n}\right)\\
            &= \sum_{j_1=1}^n \sum_{j_2=1}^n \cdots \sum_{j_n=1}^n v_{1j_1} v_{2j_2} \cdots v_{nj_n} f(e_{j_1}, e_{j_2}, \ldots, e_{j_n}).
        \end{aligned}
    \end{equation*}
        \begin{equation*}
        \begin{aligned}
            f(v_1, v_2, \ldots, v_n) &= f\left(\sum_{\sigma \in S_n} \sgn(\sigma) \prod_{i=1}^n v_{i\sigma(i)}\right)f(e_{\sigma(1)}, e_{\sigma(2)}, \ldots, e_{\sigma(n)})\\
            &= \sum_{\sigma \in S_n} \sgn(\sigma) \prod_{i=1}^n v_{i\sigma(i)}.
        \end{aligned}
    \end{equation*}
\end{proof}
    ...
    \begin{equation*}
        \det(v_1, v_2, \ldots, v_{n+1}) = \sum_{j=1}^{n+1} (-1)^{j+i} v_{ij} \det(v_2^{(i)}, \ldots, v_{j-1}^{(i)}, v_{j+1}^{(i)}, \ldots, v_{n+1}^{(i)}),
    \end{equation*}
